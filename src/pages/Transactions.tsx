...
export default function Transactions() {
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      ...
      {securities.length === 0 && !loading ? (
        ...
        <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-lg p-4 mb-6 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Harga /lembar</label>
              <input
                type="number"
                value={harga}
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Fee (Rp)</label>
              <input
                type="number"
                value={fee}
      ...
      <h2 className="text-sm font-medium text-slate-300 mb-2">Riwayat Transaksi</h2>
      {loading ? (
            ...
            <tbody>
              {transactions.map((tx) => (
                <tr key={tx.id} className="border-t border-slate-800">
                  ...
                  <td className={`py-1 pr-2 ${tx.tipe === 'buy' ? 'text-emerald-400' : 'text-red-400'}`}>
